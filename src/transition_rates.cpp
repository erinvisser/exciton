                              ...
                              double M2c, double Rpinu_val)
{
  ...
  if (tbl.built && tbl.Z_target == Z_target && tbl.A_target == A_target && tbl.M2c == M2c && tbl.Rpinu == Rpinu_val)
    return;
  ...
  // Build the OMP table using target-like KD OMP directly with
  // TALYS-correct normalization (f_v weighting in both sum1 and sum2).
  // TALYS bonetti.f90: Zix = Zindex(0,0,k0), Nix = Nindex(0,0,k0) -> target
  marley::KoningDelarocheOpticalModel omp(Z_target, A_target, 0.1);
  ...
  auto f_ws = [](double r, double R, double a) -> double
  {
    return 1. / (1. + std::exp((r - R) / a));
  };
  auto dfdr_ws = [](double r, double R, double a) -> double
  {
  ...
  for (int k = 0; k < 2; ++k)
  {
    ...
    for (int nen = -80; nen <= nenend; ++nen)
    {
      ...
      {
        double r = (static_cast<double>(i) + 0.5) * dr;
        double fv = f_ws(r, Rv, av);
        double fs = dfdr_ws(r, Rd, ad);
        double term2 = fv * r * r * dr;
        double absorption = Wv * fv - 4. * ad * Wd * fs;
